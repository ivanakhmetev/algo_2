...
class SimpleGraph:
    ...
    def WeakVertices(self):
        weak = []
        for i in range(len(self.vertex)):
            adjacency = self.m_adjacency[i]
            neighbours = [i for i in range(len(adjacency)) if adjacency[i] == 1]
            # print(neighbours)
            for j in range(len(neighbours)):
                # print(len(neighbours), j)
                current = neighbours.pop(j)
                connect = False
                # print(current, 'current')
                for k in range(len(neighbours)):
                    # print(k, 'k', neighbours)
                    connect = self.check_connect(current, neighbours[k])
        ...
        return [self.vertex[i] for i in weak]

    def check_connect(self, i, j):
        # print(self.m_adjacency[i][j], i, j)
